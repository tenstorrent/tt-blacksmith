# Phi-1 and Phi-1.5 with LoRA Experiment

This directory contains the code for the Phi-1 and Phi-1.5 model with LoRA fine-tuning experiment.
Phi-1 and Phi-1.5 model specification can be found [here](https://huggingface.co/microsoft/phi-1) and [here](https://huggingface.co/microsoft/phi-1_5), respectively.
Original LoRA paper can be found [here](https://arxiv.org/pdf/2106.09685).

## Overview

The Phi-1 and Phi-1.5 fine-tuning experiment applies the LoRA technique to adapt a pre-trained Phi-1 and Phi-1.5 model on the SST sentiment analysis dataset.
The experiment is designed to run on the Huggingface framework.

## Training

```bash
python3 blacksmith_deprecated/experiments/torch/phi/train.py
```
...
The Phi-1.5 model to be trained on Squad-V2 dataset is configured using the configuration file `phi15_squadV2.yaml`.
...
| `project_dir` | Directory for experiment outputs. | "blacksmith_deprecated/experiments/torch/phi1" |
