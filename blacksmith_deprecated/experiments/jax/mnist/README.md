...
The connection to the Tenstorrent device is established in script ```blacksmith_deprecated/tools/jax_utils.py```. For more details on connecting to Tenstorrent's hardware in JAX, please refer to [tt-xla](https://github.com/tenstorrent/tt-xla).
## Training
...
```
python3 blacksmith_deprecated/experiments/jax/mnist/single_chip/train.py
```
To run the single chip training script in Flax, run the command
```
python3 blacksmith_deprecated/experiments/jax/mnist/single_chip/train_flax.py
```
To run the multi chip training script (data or tensor parallel), run the command(s)
```
python3 blacksmith_deprecated/experiments/jax/mnist/multi_chip/data_parallel/train.py
python3 blacksmith_deprecated/experiments/jax/mnist/multi_chip/tensor_parallel/train.py
```
...
The MNIST dataset is automatically downloaded in ```blacksmith_deprecated/datasets/jax/mnist/dataloader.py``` when you run training scripts.
