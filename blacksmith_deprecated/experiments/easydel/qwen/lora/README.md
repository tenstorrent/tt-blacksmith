...
```bash
bash blacksmith_deprecated/experiments/easydel/setup_gpu_requirements.sh
```
...
```bash
python3 blacksmith_deprecated/experiments/easydel/qwen/lora/train.py \
  --config blacksmith_deprecated/experiments/easydel/qwen/lora/single_chip/qwen3_0_6b_sst2.yaml
```
...
```bash
python3 blacksmith_deprecated/experiments/easydel/qwen/lora/train.py \
  --config blacksmith_deprecated/experiments/easydel/qwen/lora/quietbox/qwen3_4b_alpaca.yaml
```
...
```bash
python3 blacksmith_deprecated/experiments/easydel/qwen/lora/train.py \
  --config blacksmith_deprecated/experiments/easydel/qwen/lora/quietbox/qwen3_4b_sst2.yaml
```
...
- `easydel_partition_specs_for_lora` in `blacksmith_deprecated/tools/jax/easydel/partitioning.py` derives the default shardings from EasyDel's built-in `partition_rules()`.
...
**SST-2** (GLUE): instruction-style prompt/response pairs padded to `max_length`, with masked labels. The Hugging Face load uses `glue` / `sst2`; `dataset_id` in the config is the logical dataset tag. The SST-2 pipeline uses the Torch `SSTDataset` loader from `blacksmith_deprecated/datasets/torch/sst2/` and formats each example as `Review: <sentence>\nOutput: {"label": "positive|negative"}`. Prompt tokens are masked with `-100` so only the response tokens contribute to the loss.

**Alpaca** (`tatsu-lab/alpaca`): general instruction-following pairs (`instruction`, optional `input`, `output`) padded to `max_length`, with masked labels. The JAX loader (`blacksmith_deprecated/datasets/jax/alpaca/alpaca_dataset.py`) wraps the Torch `AlpacaDataset` from `blacksmith_deprecated/datasets/torch/alpaca/` and produces `(input_ids, labels, attention_masks)` shaped `(num_batches, batch_size, seq_len)`. Prompt tokens are masked with `-100` so only the response tokens contribute to the loss.

`dataset_id` in the YAML selects between them via the `AVAILABLE_DATASETS` registry in `blacksmith_deprecated/datasets/jax/dataset_utils.py` (currently `"sst2"` and `"alpaca"`).
...
| `project_dir`            | Directory for experiment outputs.                             | `"blacksmith_deprecated/experiments/easydel/qwen/lora"` |
