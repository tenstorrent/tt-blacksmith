...
- **Convolutional model** (`MNISTCNN`) - A CNN with convolutional layers from `tt-blacksmith/blacksmith_xla/models/torch/mnist/mnist_cnn.py`
...
```bash
python blacksmith_xla/experiments/torch/mnist_cnn/train.py
```
...
In `blacksmith_xla/experiments/torch/mnist_cnn/single_chip/mnist_cnn.yaml` you can change values for following parameters.
...
| `project_dir` | Project directory path. | "blacksmith_xla/experiments/torch/mnist_cnn" |
