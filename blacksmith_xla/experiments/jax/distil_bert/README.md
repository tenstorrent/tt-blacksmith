...
```bash
python3 blacksmith_xla/experiments/jax/distil_bert/single_chip/train.py
```
For multi chip training (only data parallel is supported for now), run command:
```bash
python3 blacksmith_xla/experiments/jax/distil_bert/multi_chip/data_parallel/train.py
```
...
|  | `output_dir` | Directory for experiment outputs. | `"blacksmith_xla/experiments/jax/distil_bert"` |
