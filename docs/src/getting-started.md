...
> **NOTE:** Experiments that have been ported to tt-crank (the PyTorch frontend in TT-MLIR) ship a `crank/train.py` beside their `xla/` directory (e.g. `blacksmith/experiments/torch/llama/crank/train.py`); run those after `source env/activate --crank`. Until the `tt-crank` wheel is published, point at a local TT-MLIR checkout: `TT_MLIR_HOME=/path/to/tt-mlir source env/activate --crank`.
