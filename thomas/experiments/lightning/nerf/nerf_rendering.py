...
def render_rays(
    ...
    xyz_coarse, deltas_coarse = generate_ray_samples(
        rays, n_samples_coarse, near, far
    )
    
    xyz_fine, deltas_fine = generate_ray_samples(
        rays, n_samples_fine, near, far
    )
...
def calculate_coarse_rendering(
    ...
    sigmas = nerf_tree.query_coarse(xyz_coarse.reshape(-1, 3), type="sigma").reshape(
    ...
    if nerf_tree.voxels_fine is None:
        ...
        sample_densities = (
            updated_sigmas.detach()
            .squeeze()
            .clone()[valid_sample_indices[:, 0], valid_sample_indices[:, 1]]
        )
...
def calculate_fine_rendering(
    ...
    expanded_indices = important_samples.unsqueeze(1).expand(
        -1, fine_samples_per_coarse, -1
    )  # (B, scale, 2)
    ...
    if nerf_tree.voxels_fine is not None:
        with torch.no_grad():
            ...
            sample_densities = sigma_values.detach()[
                fine_indices[:, 0], fine_indices[:, 1]
            ].unsqueeze(-1)
