    ...
    *,
    ...
    def compute_loss_and_grads(
        ...
        def local_loss(lp, fs, ids, oh, lm, am):
            ...
            out = call_fn(m, ids, am)
            shift_logits = out.logits[:, :-1, :].astype(jnp.float32)
            per_token = _clamped_softmax_cross_entropy_per_token(shift_logits, oh)
            masked = per_token * lm
            num = jnp.sum(masked)
            den = jnp.sum(lm)
...
def create_eval_step_fn(
    ...
    *,
    ...
    All paths now run fully on-device. On TT the softmax-in-CE is guarded by
    ...
    On TT multi-chip (``num_devices > 1``) this uses a data-parallel
    ``shard_map`` with ``lax.psum`` aggregation across the ``data`` axis.
    On TT single-chip and on GPU/CPU it is a single ``jax.jit``.
