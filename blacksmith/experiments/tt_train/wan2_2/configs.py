...
class TrainingConfig(BaseModel):
    ...
    # "gaussian" = N(0, 1/rank) as PEFT does; "kaiming" = ttml's own init, ~4x
    # smaller at rank 32, which trains a proportionally weaker adapter.
    ...
    mesh_shape: list[int] = Field(default_factory=lambda: [4, 8])
    vae_parallel_shape: list[int] = Field(default_factory=lambda: [4, 8])
    ...
    framework: str = Field(default="tt_train")
    project_dir: str = Field(default="blacksmith/experiments/tt_train/wan2_2")
    ...
    def check_grad_clip_under_tp(self) -> Self:
        dp_size, tp_size = self.mesh_shape
        if self.grad_clip > 0.0 and tp_size > 1:
            raise ValueError(
                f"grad_clip={self.grad_clip} is not supported at TP={tp_size}: "
                f"ttml.core.clip_grad_norm is per-device, so the clip would apply to "
                f"shard-local norms instead of the global one. Set grad_clip: 0, "
