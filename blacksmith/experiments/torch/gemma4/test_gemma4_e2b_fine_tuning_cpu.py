...
"""LoRA fine-tuning for Gemma 4 E2B (text-only) — CPU baseline.

Mirror of `test_gemma4_e2b_fine_tuning_pure_torch.py` with all TT/XLA-specific
plumbing removed (no `torch.compile(backend="tt")`, no `torch_xla.sync`, no
`xr.clear_computation_cache`, no `device_manager.shard_model`). Used to
sanity-check training behaviour (e.g. loss trajectory, gradient norms) on a
plain CPU backend, away from compiler/sharding effects.
"""
import traceback
...
def _patch_embed_tokens_per_layer_split(model: torch.nn.Module) -> int:
    """Mirror the TT split-embedding workaround on CPU for parity.

    On CPU this is just two `F.embedding` calls + concat — mathematically
    equivalent to one. We keep it so the CPU baseline executes the same op
    sequence as the TT run, isolating any divergence to compiler/kernel
    behaviour rather than to the model code path itself.
    """
    n = 0
...
def _log_lora_grad_norms(model: torch.nn.Module, logger: TrainingLogger, global_step: int) -> None:
    """Print per-LoRA-weight grad norms.

    LoRA parameter names embed both the host module and the layer index, e.g.
    ``...layers.7.self_attn.q_proj.lora_A.default.weight`` — that's what we
    print, alongside the L2 norm of its accumulated gradient. Called right
    before ``optimizer.step()`` so the grads are the full accumulated value.
    """
    rows = []
    for name, p in model.named_parameters():
        ...
        if "lora_" not in name:
            continue
        rows.append((name, tuple(p.shape), p.grad.detach().norm().item()))

    if not rows:
        return

    logger.info(f"--- LoRA grad norms @ optimizer step {global_step} ---")
    name_w = max(len(n) for n, _, _ in rows)
    for name, shape, gn in rows:
        logger.info(f"  {name:<{name_w}s}  shape={shape!s:<14s}  ||g||={gn:.4e}")
...
def train(
    ...
    logger.info("Starting training (CPU baseline)...")
    ...
    try:
        ...
        for epoch in range(config.num_epochs):
            ...
            for batch in tqdm(train_dataloader, desc="Training"):
                ...
                batch = {
                    "input_ids": batch["input_ids"].to(device_manager.device),
                    "attention_mask": batch["attention_mask"].to(device_manager.device),
                    "expected_output": expected_output.to(device_manager.device),
                    "labels_mask": labels_mask.to(device_manager.device),
                }
                ...
                if accumulation_step == config.gradient_accumulation_steps:
                    if global_step % config.steps_freq == 0:
                        _log_lora_grad_norms(model, logger, global_step)
...
if __name__ == "__main__":
    default_config = Path(__file__).parent / "test_gemma4_e2b_wizardlm_cpu.yaml"
    args = parse_cli_options(default_config=default_config)
    config: TrainingConfig = generate_config(TrainingConfig, args.config, args.test_config, args.test_checkpoint_path)

    assert not config.use_tt, (
        "test_gemma4_e2b_fine_tuning_cpu.py expects use_tt=False; "
        "use test_gemma4_e2b_fine_tuning_pure_torch.py for the TT run."
    )
