...
"""CLI / config loading for the tt-crank scripts.

Copy of `blacksmith.tools.cli`. It is duplicated because the original applies the nested
pytest defaults with `issubclass(config, <tt-xla TrainerConfig>)`, which is both an import of
the tt-xla trainer package and false for the tt-crank `TrainerConfig` copy.
"""
import argparse
import os
