...
def resolve_targets(target_set: str) -> list[str]:
    try:
...
def _iter_lora_params(model: Any) -> Iterator[tuple[str, Any]]:
...
def _gather(tensor: Any, name: str, mesh_shape: Sequence[int]) -> np.ndarray:
...
def _scatter(w_np: np.ndarray, name: str, mesh_shape: Sequence[int]) -> Any:
...
def init_lora_A_gaussian(model: Any, rank: int, mesh_shape: Sequence[int] = (1, 1), seed: int = 0) -> int:
...
def lora_state_dict(model: Any, mesh_shape: Sequence[int] = (1, 1)) -> dict[str, np.ndarray]:
...
def save_lora_expert(model: Any, path: str, mesh_shape: Sequence[int] = (1, 1)) -> int:
...
def load_lora_expert(model: Any, path: str, mesh_shape: Sequence[int] = (1, 1)) -> int:
...
def save_all(experts: dict[str, Any], config: Any, suffix: str = "", logger: Any = None) -> None:
    for role, model in experts.items():
        path = _with_suffix(config.expert_path(role), suffix)
        n = save_lora_expert(model, path, config.mesh_shape)
        if logger is not None:
            logger.info(f"[save] wrote {n} LoRA tensors -> {path}")


def load_all(experts: dict[str, Any], config: Any, suffix: str = "", logger: Any = None) -> None:
    for role, model in experts.items():
        path = _with_suffix(config.expert_path(role), suffix)
        n = load_lora_expert(model, path, config.mesh_shape)
        if logger is not None:
            logger.info(f"[load] restored {n} LoRA tensors <- {path}")
